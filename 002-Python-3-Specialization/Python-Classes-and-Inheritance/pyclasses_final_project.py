...
class WOFHumanPlayer(WOFPlayer):
  ...
  def getMove(self, category, obscuredPhrase, guessed ):
    ...
    print("")
    ## show board from pre def. helper functions ##
    showBoard()
    assert showBoard() is str # unit test: verify response of showBoard 
    ## prompt user ##
    prompt = "Guess a letter, phrase, or type 'exit' or 'pass':"
    userInput = input(prompt) ## inputs will be enforced later
...
class WOFComputerPlayer(WOFPlayer):
  ...
  def __init__(self, name, difficulty):
    # call .super()
    WOFPlayer.__init__(self, name) # call the parent class's constructor
    self.name = name
  ...
  def smartCoinFlip(self):
    move_score = random.randint(1, 10)
    if move_score <= self.difficulty:
      return True
    else: return False

  def getPossibleLetters(self, guessed):
    """returns list of guessable letters"""

    # list of possibles excluding guessed
    possible_letters = []
    for alpha in LETTERS:
      if alpha not in guessed:
        possible_letters.append(alpha)

    # second conditional - exclude vowels if funds insufficient
  ...
  def getMove(self, category, obscuredPhrase, guessed):
    """return a valid move, or pass if no possibles"""
    possibles = self.getPossibleLetters(guessed)
    assert type(possibles) == list
    if possibles == []:
      return "pass"
    else: 
      # True scenario
      if self.smartCoinFlip() is True:
        ...
        for char in self.SORTED_FREQUENCIES[::-1]:
          if char in possibles:
            return str(char)
    
      else: return str(random.choice(possibles))
